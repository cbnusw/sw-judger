...
export class IoSetControlComponent implements ControlValueAccessor, OnInit {
  ...
  uploadIoSet(files: File[]): void {
    ...
    from(uploads).pipe(
      ...
      () => {
        const inList = uploadResponses.filter(
          file => this.inExt ? file.filename.endsWith(this.inExt) : file.filename.split('.').length === 1
        );
        const outList = uploadResponses.filter(
          file => this.outExt ? file.filename.endsWith(this.outExt) : file.filename.split('.').length === 1
        );

        this.value = [...this.value, ...inList.map(inFile => {

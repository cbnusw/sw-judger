...
const compile_c = function (name, submitId) {
  ...
  try {
    execSync(`mkdir ${rootSubmitId} && gcc -o ${out_path} ${path.join(CODE_BASE_PATH, name)} -lm`);
  } catch (err) {
    console.log(err);
    out_path = 'null';
  }
...
const compile_java = function (name, originalName, submitId) {
  ...
  try {
    execSync(
      `mkdir ${rootSubmitId} && cp ${path.join(CODE_BASE_PATH, name)} ${path.join(
        rootSubmitId,
        originalName
      )} &&javac ${path.join(rootSubmitId, originalName)} -encoding UTF8 &&rm -rf ${path.join(
        rootSubmitId,
        originalName
      )}`
    );
  ...
  try {
    execSync(
      `mkdir ${rootSubmitId} && cp ${path.join(CODE_BASE_PATH, name)} ${path.join(
        rootSubmitId,
        originalName
      )} && /kotlin/bin/kotlinc-jvm ${path.join(rootSubmitId, originalName)} -include-runtime -d ${path.join(
        rootSubmitId,

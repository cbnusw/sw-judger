...
const startJudge = async (submitId) => {
  ...
  switch (config["language"]) {
    ...
    case "java":
      ...
      config["args"] = `-cp /${submitId} ${originalJava.filename.substring(
        0,
    ...
    case "kotlin":
      ...
      compiledPath = base.compile_kotlin(
        config["code_name"],
        originalKotlin.filename,
        submitId
      );
      config["exe_path"] = "/kotlin/bin/kotlin";
      config["args"] = `/${submitId}/${
        originalKotlin.filename.substring(
          0,
          originalKotlin.filename.lastIndexOf(".")
        ) + ".jar"
      }`.split(" ");
      config["memory_limit_check_only"] = 1;
  ...
  if (result["type"] == judger.RESULT_WRONG_ANSWER) result["type"] = 5;
...
const judge = async (config, submit) => {
  ...
  for (const io of ioSet) {
    config["input_path"] = path.join(
      CODE_BASE_PATH,
      getBasename(io.inFile.url)
    );
    config["answer_path"] = path.join(
      CODE_BASE_PATH,
      getBasename(io.outFile.url)
    );
    config["output_path"] = path.join(
      "/" + submit._id,
      `${config["submit_id"]}.out`
    );

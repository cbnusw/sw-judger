...
const {
  ...
  IS_NOT_TEST_PERIOD, IS_NOT_CONTESTANT,
} = require('../../../../errors');
const {isAssigned} = require("../problem/service");
...
const handleAccessAssignmentProblems = asyncHandler(async (req, res, next) => {
  ...
  if (!assignment.students.map(student => String(student)).includes(String(user.info))) return next(IS_NOT_CONTESTANT);
